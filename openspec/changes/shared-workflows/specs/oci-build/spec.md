...
each system in `inputs.systems`, runs `nix flake check -L`, builds and uploads coverage to
Codecov (using `codecov-action@v6`), builds the OCI image via `nix build
.#packages.<system>.docker`, and — when `push_oci` is true and the build is not from a fork
PR — pushes the image to Docker Hub and ghcr.io. A follow-up `oci-manifest` job SHALL
assemble a multi-arch manifest and push it.

#### Scenario: PR from the owner with `push_oci: true`

- **WHEN** invoked from a non-fork PR with `systems: '["x86_64-linux","aarch64-linux"]'`,
  `images: "kalbasit/foo"`, `push_oci: true`
...
- **THEN** the workflow SHALL still run flake-check and the OCI image build (read-only steps)
  on every arch; SHALL skip codecov upload, registry login, push, and the `oci-manifest` job
