...
dependency lock state, recomputes the Nix dependency hash for each listed package, rewrites
the matching hash field in the package's `default.nix`, and auto-commits the result.

#### Scenario: Go consumer with one package and out-of-date hash

- **WHEN** invoked with `languages: '{"go":{"packages":["myapp"]}}'` and the `go.sum` of the
  consumer repo has changed
...
#### Scenario: Go consumer with multiple packages, all up to date

- **WHEN** invoked with `languages: '{"go":{"packages":["a","b","c"]}}'` and all hashes match
- **THEN** for each package the workflow SHALL run `nix build .#<pkg>.goModules` and then
  `nix build .#<pkg>.goModules --rebuild` to confirm; SHALL emit `::notice::Hash is up to
  date for <pkg>`; SHALL NOT create a commit
...
command (e.g., `poetry lock`), and (3) a per-package "build / capture hash / rewrite
default.nix" recipe. No change to the workflow's input shape or to other languages'
...
- **THEN** existing Go-only consumers SHALL continue to work unchanged; consumers opting in
  by adding `"python": {...}` to their `languages` input SHALL get Python hash regeneration
